...
interface PromptModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { id?: string; title: string; content: string; companyId: string; isOfficial: boolean; folderId?: string }) => void;
...
export const PromptModal: React.FC<PromptModalProps> = ({ isOpen, onClose, onSave, initialData, currentUser, companies, isOfficial = true }) => {
  ...
  const handleSave = () => {
    onSave({
      ...
      title,
      content,
      companyId: isOfficial ? companyId : '',
      isOfficial: initialData ? initialData.isOfficial : isOfficial,
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
      <div className="bg-white rounded-3xl p-6 w-[480px] shadow-2xl border border-slate-100">
        ...
        <div className="space-y-5">
          ...
          {currentUser.role === 'master_admin' && isOfficial && (
            <div>
